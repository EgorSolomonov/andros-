...
var core = (function () {
  return {
    ...
    uiFormBlock: function (obj) {
      ...
      const buttonElement =
        settings.button && form.querySelector(settings.button);
    ...
    getFormValidation: function (el) {
      ...
      inputs.forEach((input) => {
        ...
        if (isError) {
          const hintContainer = input
            .closest(".form__group")
            .querySelector(".form__group_hints");
          const hintError = input
            .closest(".form__group")
            .querySelector(".form__group_hints .hint__item--error");
          if (hintContainer) {
            if (!hintError) {
...
const componentsApp = (function () {
  return {
          ...
          el.querySelectorAll("*[data-tab]").forEach((tab) => {
            tab.addEventListener("click", () => {
              ...
              if (
                ...
                core.getViewPort().width < settings.breakpoint
              ) {
      ...
      if (el) {
        el.forEach((item) => {
          item.addEventListener("click", () => {
            ...
            const subheader =
              document.querySelector(".section--submenu").offsetHeight / 2;
    ...
    initFloatActions: function () {
      ...
      let startX, startY;
      const swipeElement = document.querySelector(
        ".js-float-actions .r_toast__item"
      );
...
const menuApp = (function () {
  return {
    ...
    hoverIntent: function () {
      let uiCatalogDropdowns = document.querySelectorAll(
        ".js-mainmenu .mainMenu__item.is-dd"
      );
    ...
    default: function () {
      ...
      const mainSubmenuLinks = document.querySelectorAll(
        ".js-mainmenu .nav--catalog .nav__item.is-dd"
      );

      if (core.isTouchDevice() == true || core.getViewPort().width < 1200) {
        mainMenuLinks.forEach(function (item) {
          item
            .querySelector(".mainMenu__item_link")
            .addEventListener("click", menuHandlerLevelOne);
        });
        mainSubmenuLinks.forEach(function (item) {
          item
            .querySelector(".nav__item_link")
            .addEventListener("click", menuHandlerLevelTwo);
        });
      }

      document
        .querySelectorAll(".sidebar .nav--catalog .nav__item.is-dd")
...
function menuHandlerLevelOne(e) {
  ...
  let elements = [
    ...document.querySelectorAll(".js-mainmenu .mainMenu__item.is-dd "),
  ];
...
function menuHandlerLevelTwo(e) {
  ...
  let item = e.target.parentElement;
  let elements = [
    ...document.querySelectorAll(".js-mainmenu .nav--catalog .nav__item.is-dd"),
  ];
  let otherElements = elements.filter(function (element) {
    return element !== item;
  });
...
const appointmentApp = (function () {
  ...
  const toggleSections = (showStart, showOnline) => () => {
    document
      .querySelector(selectors.sectionStart)
      .classList.toggle("is-shown", showStart);
    document
      .querySelector(selectors.sectionOnline)
      .classList.toggle("is-shown", showOnline);
  };
  ...
  const addClickListener = (element, showStart, showOnline) => {
    ...
    if (targetElement) {
      targetElement.addEventListener(
        "click",
        toggleSections(showStart, showOnline)
      );
    }
  };
...
const formHandlerApp = (function () {
  ...
  const initSubmit = (el) => {
    ...
    if (
      formSelector === ".js-form-search" ||
      formSelector === ".js-form-search-popup"
    ) {
...
const benefitsPlasticCarousel = document.querySelectorAll(
  ".js-benefits-plastic-carousel"
);
