...
foreach ($arResult["SECTIONS"] as $arSection) {
	...
	while ($element = $arElements->GetNext()) {
		...
		$res = CIBlockElement::GetProperty($arParams["IBLOCK_ID"], $elementId);
		while ($arProperty = $res->Fetch()) {
			$elementProperties[$arProperty["CODE"]] = $arProperty["VALUE"];
		}

		// Добавляем свойства элемента в массив меню
